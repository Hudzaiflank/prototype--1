...
export function RoomMonitorPage() {
  ...
  useEffect(() => {
    if (Number(game?.id) !== Number(sessionId) || game?.status !== "FINISHED") {
  ...
  useEffect(() => {
    if (
      ...
      return;
    gameApi
      .currentTurn(sessionId, selectedGroupId)
      .then(({ data }) => setCurrentTurn(data.data))
      .catch(() => setCurrentTurn(null));
  }, [game?.status, selectedGroupId, sessionId]);

  const refreshMonitor = async () => {
    const [{ data: stateResponse }, { data: groupsResponse }] =
      await Promise.all([gameApi.state(sessionId), gameApi.groups(sessionId)]);
  ...
  const refreshCurrentTurn = async (groupId = selectedGroupId) => {
    if (!sessionId || !groupId || !["PLAYING", "PAUSED"].includes(game?.status))
      return;
    try {
  ...
  const handleAction = async (action) => {
    ...
    try {
      ...
      if (action === "reveal-cards") {
        if (!selectedGroupId || !currentTurn)
          throw new Error("Belum ada turn aktif.");
  ...
  return (
    <section className="space-y-6" aria-labelledby="monitor-title">
        ...
        >
          <div className="w-full max-w-sm rounded-2xl border border-amber-300/60 bg-slate-900 p-8 text-center shadow-2xl">
            ...
            <p className="mt-3 text-sm text-slate-300">
              Anda akan diarahkan ke dashboard Guru.
            </p>
            <p
              className="mt-6 text-6xl font-black text-amber-300"
              aria-live="assertive"
            >
              {completionCountdown}
            </p>
      ...
      {groups.length && teacherInputMode ? (
        <div className="rounded-2xl border border-amber-300/40 bg-slate-950/50 p-5">
          <p className="text-lg font-semibold text-amber-200">
            Sisa{" "}
            {Math.max(
              Number(groups[0].totalTurnCount ?? 0) -
                Number(groups[0].completedTurnCount ?? 0),
              0,
            )}{" "}
            turn
          </p>
        ...
        <div className="space-y-4 rounded-2xl border border-slate-800 bg-slate-950/50 p-5">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">
              Pilih kartu kelompok untuk mengatur turn yang sedang dipantau.
            </p>
          </div>
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {groups.map((group) => (
              <button
                className={`rounded-xl border p-4 text-left ${String(group.id) === String(selectedGroupId) ? "border-amber-300" : "border-slate-700"}`}
                type="button"
                key={group.id}
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="text-xs text-slate-400">{group.status}</span>
                </div>
                <p className="mt-2 text-sm text-slate-400">
                  Turn{" "}
                  {Number(
                    group.currentTurnNumber ?? group.completedTurnCount ?? 0,
                  )}{" "}
                  dari {Number(group.totalTurnCount ?? 0)}
                </p>
                <p className="mt-1 text-sm text-amber-200">
                  Sisa{" "}
                  {Math.max(
                    Number(group.totalTurnCount ?? 0) -
                      Number(group.completedTurnCount ?? 0),
                    0,
                  )}{" "}
                  turn
                </p>
      ...
      <GameBoard
        state={
          currentTurn?.participantCardState === "REVEALED"
            ? "revealed"
            : "hidden"
        }
        participant={
          currentTurn?.participantName ?? currentTurn?.participant?.displayName
        }
