...
export function RoomConfigurePage() {
  ...
  const createSession = async (event) => {
    ...
    try {
      ...
      if (inputMode === "STUDENT") {
        navigate(`/teacher/rooms/${roomId}/monitor?sessionId=${createdSessionId}`);
      } else {
  ...
  const confirmParticipantImport = async () => {
    ...
    try {
      const activeSessionId = await ensureTeacherSession();
      await gameApi.importTeacherParticipants(activeSessionId, participantPreview.rows.map(({ fullName, content }) => ({ fullName, content })));
      setParticipantPreview(null);
      setError("");
    } catch (requestError) {
      setError(requestError.response?.data?.message ?? requestError.message ?? "Data Excel belum dapat diimport.");
    } finally {
      setImporting(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      {inputMode === "TEACHER" ? (
          <p className="text-sm text-slate-400">Semua data permainan akan dikendalikan oleh Guru.</p>
      ) : (
        <label className="block text-sm font-medium">
          Mode permainan
          <select
      ...
      {inputMode === "TEACHER" ? (
        <div className="space-y-4 rounded-xl border border-amber-300/40 p-4">
          ...
          <div className="grid gap-3 md:grid-cols-[1fr_2fr_auto]">
            ...
            <input className="rounded-lg border border-slate-700 bg-slate-900 px-3 py-3" maxLength="500" placeholder="Permasalahan (maksimal 500 karakter)" value={problemContent} onChange={(event) => setProblemContent(event.target.value)} required />
            <button className="rounded-lg bg-amber-300 px-4 py-3 font-bold text-slate-950 disabled:opacity-50" type="button" onClick={addParticipant} disabled={!sessionId}>Tambah data</button>
          </div>
          <div className="flex flex-wrap gap-3">
            <button className="rounded-lg border border-slate-600 px-4 py-3 text-sm font-bold" type="button" onClick={downloadParticipantTemplate}>Download template Excel</button>
          ...
          {participantPreview ? <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr><th className="px-3 py-2">Baris</th><th className="px-3 py-2">Nama</th><th className="px-3 py-2">Permasalahan</th><th className="px-3 py-2">Status</th></tr></thead><tbody>{participantPreview.rows.map((row) => <tr key={row.row}><td className="px-3 py-2">{row.row}</td><td className="px-3 py-2">{row.fullName}</td><td className="px-3 py-2">{row.content}</td><td className="px-3 py-2 text-emerald-300">Valid</td></tr>)}{participantPreview.errors.map((item) => <tr key={`error-${item.row}`}><td className="px-3 py-2">{item.row}</td><td colSpan="2" className="px-3 py-2">{item.message}</td><td className="px-3 py-2 text-rose-300">Error</td></tr>)}</tbody></table><button className="mt-3 rounded-lg bg-amber-300 px-4 py-2 font-bold text-slate-950 disabled:opacity-50" type="button" disabled={!participantPreview.valid || importing} onClick={confirmParticipantImport}>Konfirmasi import</button></div> : null}
          {sessionId ? <button className="rounded-lg bg-emerald-400 px-4 py-3 font-bold text-slate-950" type="button" onClick={() => navigate(`/teacher/rooms/${roomId}/monitor?sessionId=${sessionId}`)}>Lanjut ke permainan</button> : null}
        </div>
      ) : null}
      {!sessionId ? <button
