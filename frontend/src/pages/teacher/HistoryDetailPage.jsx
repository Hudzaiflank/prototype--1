...
export function HistoryDetailPage() {
  ...
  useEffect(() => {
    historyApi
      ...
      .catch((requestError) => setError(requestError.response?.data?.message ?? "Detail history belum dapat dimuat."));
  }, [sessionId]);
  ...
  return (
    <section className="space-y-6" aria-labelledby="history-detail-title">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.28em] text-amber-300">Teacher space</p>
          <h1 className="mt-3 text-3xl font-bold" id="history-detail-title">Detail history permainan</h1>
        </div>
        <Link className="text-sm text-amber-200" to="/teacher/history">Kembali ke history</Link>
      </div>
