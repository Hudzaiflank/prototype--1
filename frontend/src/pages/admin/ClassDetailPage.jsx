...
export function ClassDetailPage() {
  ...
  const update = async (event) => {
    ...
    try {
      await classApi.update(classId, {
        ...form,
        major: isSmp ? "" : form.major,
        classNumber: Number(form.classNumber),
      });
      load();
    } catch (requestError) {
      setError(
        requestError.response?.data?.message ?? "Kelas belum dapat diubah.",
      );
    }
  };
  const assign = async () => {
    ...
    try {
      ...
      setError(
        requestError.response?.data?.message ?? "Guru belum dapat di-assign.",
      );
    }
  };
  const remove = async (teacherId) => {
    try {
      ...
      setError(
        requestError.response?.data?.message ??
          "Assignment belum dapat dihapus.",
      );
    }
  };
  const resetStudents = async () => {
    ...
    try {
      ...
      setError(
        requestError.response?.data?.message ?? "Siswa belum dapat direset.",
      );
    }
  };
  return (
    <section className="space-y-6" aria-labelledby="class-detail-title">
      ...
      {form ? (
        <form
          className="grid gap-3 rounded-2xl border border-slate-800 bg-slate-950/50 p-5 md:grid-cols-4"
          onSubmit={update}
        >
          <input
          ...
          <input
            ...
            onChange={(event) =>
              setForm({ ...form, classNumber: event.target.value })
            }
            required
          />
          <div className="flex gap-2">
            <select
              className="min-w-0 flex-1 rounded-lg border border-slate-700 bg-slate-900 px-3 py-3"
              value={form.status}
              onChange={(event) =>
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-950/50 p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            className="rounded-lg border border-rose-300 px-3 py-2 text-sm text-rose-300"
            type="button"
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {students.map((student) => (
            ...
            >
              {student.fullName}{" "}
              <span className="text-slate-400">- {student.nisn}</span>
            </div>
          ))}
        </div>
      </div>
      <div className="rounded-2xl border border-slate-800 bg-slate-950/50 p-5">
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          ...
          >
            ...
            {teachers
              .filter(
                (teacher) =>
                  !classData?.assignedTeachers?.some(
                    (assigned) => assigned.id === teacher.id,
                  ),
              )
              .map((teacher) => (
        ...
        <div className="mt-4 space-y-2">
          {classData?.assignedTeachers?.length ? (
            classData.assignedTeachers.map((teacher) => (
              <div
