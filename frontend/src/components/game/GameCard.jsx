...
export function GameCard({ type, state, content, onClick }) {
  ...
  return (
    <article
      ...
      role={type === "problem" && state === "revealed" && onClick ? "button" : undefined}
      tabIndex={type === "problem" && state === "revealed" && onClick ? 0 : undefined}
      onClick={type === "problem" && state === "revealed" ? onClick : undefined}
      onKeyDown={(event) => {
        if (type === "problem" && state === "revealed" && onClick && ["Enter", " "].includes(event.key)) {
          event.preventDefault();
          onClick();
        }
      }}
    >
      <div className={`game-card__inner ${state === "revealed" ? "is-revealed" : ""}`}>
        ...
        <div className={`game-card__face game-card__front ${isParticipant ? "" : "is-problem"}`}>
          <span className="game-card__inner-frame" />
          <span className="game-card__foil" />
          <span className="game-card__corner"><b>{rank}</b><small>{suit}</small></span>
          ...
          <span className="game-card__corner game-card__corner--bottom"><b>{rank}</b><small>{suit}</small></span>
        </div>
      </div>
    </article>
  );
