...
export async function createTopic({
  ...
  const [rows] = await pool.execute(
    `SELECT id, title, description, visibility, created_by AS createdBy, created_at AS createdAt
...
export async function updateTopic({
  ...
  const [result] = await pool.execute(
    `UPDATE topics SET title = ?, description = ?, visibility = ?, updated_at = CURRENT_TIMESTAMP
     ...
     [title, description ?? null, visibility, topicId, schoolId, role, userId],
  );
